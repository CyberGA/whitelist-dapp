...
export default function Home() {
  ...
  const addAddressToWhielist = async () => {
    try {
  ...
  const getNumberOfWhitelisted = async () => {
    try {
      ...
      setNumberOfWhitelisted(prev => _numberOfWhitelisted);
    } catch (error) {
      console.error(error)
    }
  }
  ...
  /**
   * connect wallet: Connect the MetaMask wallet
   */
  ...
  const renderButton = () => {
    if (walletConnected) {
      if (joinedWhitelist) {
        ...
        return (<button onClick={addAddressToWhielist} className={styles.button}>
          Join the Whitelist
        </button>)
      }
    } else {
...
Home.title = "Whitelist "
Home.desc = "Whitelist-Dapp"